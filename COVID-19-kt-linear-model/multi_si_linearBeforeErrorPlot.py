...
class MultiSILinear:
    ...
    def __init__(self, infection_file, pop_file, rname_file, travel_file, k, jp, alpha, data_end_date,lapath,start,region_code,forecastDays,split):
        ...
        self.split = split
        # print('self.avgDiagonal: ',self.avgDiagonal )
        # print(self.zero_diagMat)	
        # print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
        # print(self.norm_diag)
    ...
    def learn_betas(self, split = -1):
        ...
        betas,Xj = self.ind_beta(X, Y, self.alpha, self.k, split - 1, self.pops, self.jp)
        #no normalization, no diagonal
        X1, Y1 = self.data_prep(self.I[:, 0: split], self.zero_diagMat, self.pops, self.k, self.jp, with_travel)
        betas1,Xj1 = self.ind_beta(X1, Y1, self.alpha, self.k, split - 1, self.pops, self.jp)
        #with normalization, no diagonal, inandout
        X2, Y2 = self.data_prep(self.I[:, 0: split], self.norm_diag, self.pops, self.k, self.jp, with_travel)
        # print('X2[50]',X2[50])
        # print('X2 len',len(X2))
        # print('X2[50] len',len(X2[50]))
        # print('X2[50][0] len',len(X2[50][0]))
        # print('X2[50][0]',X2[50][0])
        # print('X2[50][3]',X2[50][3])
        # print('Y2[50]',Y2[126])
        # print('Y2 len',len(Y2))
        # print('Y2[50]',Y2[50])
        # print('Y2[50] len',len(Y2[50]))
        # print('Y2[50][0]',Y2[50][0])
        betas2,Xj2 = self.ind_beta(X2, Y2, self.alpha, self.k, split - 1, self.pops, self.jp)
        ...
        betas5,Xj5 = self.ind_beta(X5, Y5, self.alpha, self.k, split - 1, self.pops, self.jp) 

        print("beta 2",betas2)
        print("beta 5", betas5)

        return betas, betas0, betas1, betas2,betas3,betas4, betas5, X2, Xj2


    def forecast(self, betas,betas0,betas1,betas2,betas3,betas4,betas5, forward, split = - 1):
        """
        ...
        return infections, infections0,infections1,infections2, infections3,infections4, infections5 ,yt2
...
if __name__ == '__main__':
    # Names of the input data files
    # inf_file = '../../data/global_data_4_13.txt'
    # pop_file = '../../data/global_population_data.txt'
    # region_name_file = '../../data/countries_list2.txt'
    # travel_mat_file = '../../data/global_travel_data.txt'
    ...
    betas, betas0, betas1, betas2,betas3,betas4,betas5, X2, Xj2 = model.learn_betas(split)
    # print('betas2',betas2)
    ...
    predictions, predictions0, predictions1, predictions2,predictions3,predictions4,predictions5,yt2 = model.forecast(betas,betas0,betas1,betas2,betas3,betas4,betas5,forecastDays, split) # Predict n days after the training point
